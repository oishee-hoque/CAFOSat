...
class CAFODataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if self.mode == "folder":
            ...
            self.val_set = JSONImageDataset(self.data_path, split="test", transform=self.transform)
            self.class_names = ["non_cafo", "cafo"]
        else:
            raise ValueError(f"Unsupported mode: {self.mode}")

    def train_dataloader(self):
        return DataLoader(self.train_set, batch_size=self.batch_size, num_workers=self.worker, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self.val_set, batch_size=self.batch_size, num_workers=self.worker)
